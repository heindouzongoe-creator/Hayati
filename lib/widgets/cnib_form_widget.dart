...
class _CnibFormWidgetState extends State<CnibFormWidget> {
  ...
  @override
  void dispose() {
    _numeroCtrl.dispose(); _nomCtrl.dispose(); _prenomCtrl.dispose();
    _dateNaissCtrl.dispose(); _lieuNaissCtrl.dispose(); _dateExpirCtrl.dispose();
    super.dispose();
  }
  ...
  bool get _champsVerrouilles => _scanFait && widget.verrouille;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Text('Informations CNIB *', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
  ...
  Widget _champCnib({required TextEditingController ctrl, required String label, required IconData icone, String? hint, bool majuscule = false}) {
    return TextFormField(
      controller: ctrl,
      textCapitalization: majuscule ? TextCapitalization.characters : TextCapitalization.words,
      onChanged: (_) => _notifier(),
      readOnly: _champsVerrouilles,
